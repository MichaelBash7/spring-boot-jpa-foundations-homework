...
public class UserDaoImpl implements UserDao{
    ...
    public User findById(long id) {
        ...
        return jdbc.queryForObject("select u.id as UID, name, age, p.id as PID, species from user u, pets p where u.id = :id " +
                "and u.pet_id = p.id", params, new UserMapper());
    }

    @Override
    public List<User> findAll() {
        return jdbc.query("select u.id as UID, name, age, p.id as PID, species from user u, pets p where u.pet_id = p.id", new UserMapper());
    }
